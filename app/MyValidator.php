...
class MyValidator extends DBcon {
    //プロパティ
    private $date;
    private $errors = [];
    ...
    public function validateForm(){
        ...
        foreach(self::$fields as $field){
            ...
            if(!array_key_exists($field,$this->date)){
                //なければエラーメッセージを表示
    ...
    private function validatorEmail(){
        $val = trim($this->date['email']);

        if(empty($val)) $this->addEmptyError('email','メールアドレス');

        if(!empty($val)) $this->duplicateCheck($val, 'email', 'メールアドレス');
    
    }

    private function validatorPassword(){
        $val = trim($this->date['password']);
    ...
    private function regexCheck(string $value, string $name, string $pattern){
        ...
        //重複確認メソッド
    public function duplicateCheck($val,string $key, string $name){     
        try{
            $db = parent::getDb();
            //条件内でemailの値が一致したmembersテーブルのemailの件数をすべて取得しcntというカラムで取得
            $member = $db->prepare('SELECT COUNT(*) AS cnt FROM members WHERE :email');
            //フォームから受け取ったメールアドレスのデータをemailに代入
            $member->bindValue(':email',$val);
            //POSTのemailがデータベース内に入ってれば1、入っていなければ0になる
            $member->execute();
            //[cnt => 0 or 1]が$recodeに代入される
            $recode = $member->fetch();
                if($recode['cnt'] > 0){
                    $this->errors[$key] = "{$name}が重複しています";
                }
            }catch(PDOException $e){
                 $this->$errors[$key] = "エラーメッセージ:{$e->getMessage()}";
            } 
...
//     private function checkNull(array $date){
//         foreach($date as $key => $value){
